...
export function PendingReviewsList() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (!response.ok) {
          throw new Error('Failed to fetch PRs needing review');
        }
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="text-center text-red-600 dark:text-red-400">
            Error loading PRs needing review: {error}
          </div>
        </CardContent>
      </Card>
    );
  ...
  const prsNeedingReview = data?.reviewMetrics?.prsNeedingReview || [];
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          PRs Needing Review ({prsNeedingReview.length})
        </CardTitle>
        <CardDescription>
          Pull requests waiting for initial review or feedback
        </CardDescription>
      </CardHeader>
      <CardContent>
